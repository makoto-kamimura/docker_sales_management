...
| 4 | AIコンシェルジュ | チャット形式での商品提案・問い合わせ対応 (Dify連携) | Web/Mobile: 全画面右下チャット <br/> API: `POST /api/v1/ai_concierge/messages` |
| 5 | 商品検索 | キーワード/カテゴリ(id・slug)/価格/タグ検索 (pg_trgm + pgvector) | Web/Mobile: `/search` <br/> API: `GET /api/v1/products?q=...&category_slug=...` |
...
| 7 | 整備予約 / システム開発依頼 | 整備(`maintenance`)の予約、システム(`system`)の取付・開発依頼の受付と進捗管理 | Web: `/maintenance`, `/system`, `/requests` <br/> API: `POST /api/v1/service_requests`, 管理: `PATCH /api/v1/admin/service_requests/:id` |

カテゴリは **coffee / parts** (物販) と **maintenance / system** (サービス) で構成する。サービス系はカート購入ではなく依頼フロー(要件7)へ誘導する。
...
- **ゲスト**: 商品閲覧/検索/AIコンシェルジュのみ
- **会員 (member)**: + 注文/サブスク/プロフィール/整備予約・開発依頼
- **管理者 (admin)**: + 販売管理/在庫操作/整備予約・開発依頼の受付管理
